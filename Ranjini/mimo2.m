...
figure();
plot(data_tx(1:40),'ro');
hold on;
stairs(data_tx(1:40),'k');
ylim([-.5 1.5]);
title('Data Genration');
xlabel('Time');
ylabel('amplitude');
...
% STBC mapper
% Input
  [r c]=size(ofdm_symbols);X1=[];X2=[];X3=[];X4=[];
for re=1:c
...
for itr=1:100
    ...
    %H_up2 = (raylrnd((1/sqrt(2)),ntx,nrx,n) + 1j*raylrnd((1/sqrt(2)),ntx,nrx,n))*1; % Rayleigh channel
    H_down1= conj(H_up(:,2,:));     %
    H_down2= -conj(H_up(:,1,:)); 
    H_down3=  conj(H_up(:,4,:));
    H_down4= -conj(H_up(:,3,:));%
    ...
    H_down11=-(H_up(:,2,:));%
    H_down12=(H_up(:,1,:));
    % changing the channel as orthogonal
    %H_up=[H_up1 H_up2];
    H_downi=[H_down1 H_down2 H_down3 H_down4]; 
    H_downj=[H_down5 H_down6 H_down7 H_down8]; 
    H_downk=[H_down9 H_down10 H_down11 H_down12];
    H=[H_up;H_downi;H_downj;H_downk];
     N = ((randn(nrx,n) + 1j*randn(nrx,n)))*(1/snr(snri)); % white gaussian noise, 0dB variance
     %Nj = ((randn(nrx,n) + 1j*randn(nrx,n)))*(1/snr(snri));
     N1=N(1,:);
    N2=N(2,:);
    N3=N(1,:);
    N4=N(2,:);
...
% receiver Y=XH+N
 for t=1:n
    ...
    for i=1:n
        ...
        r_cap(3,i)=x(3,i)/HHH(3,3,i);
        r_cap(4,i)=x(4,i)/HHH(4,4,i);
    end
    ...
    for i=1:n/seg
        ...
        temp3=r_cap(3,((seg*(i-1)+1):i*seg));
        temp4=r_cap(4,((seg*(i-1)+1):i*seg));
        temp=[temp temp4.' temp3.' temp2.' temp1.'];
    end
