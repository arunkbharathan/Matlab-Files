...
function [bestAlpha bestScore] = TRSPECCSPWithBestParams(trainingEEGSignals, alphaList, k, nbFilterPairs,i,j)
%
...
for alpha = alphaList        
    ...
    for iter=1:k
        ...
        CSPMatrix = TRSpeccsp(subTrainingSets{iter},i,j, alpha);
        featureTrain = extractCSPFeatures(subTrainingSets{iter}, CSPMatrix, nbFilterPairs);
        LDAModel = LDA_Train(featureTrain);            
        featureTest = extractTRCSPFeatures(validationSets{iter}, CSPMatrix, nbFilterPairs);
